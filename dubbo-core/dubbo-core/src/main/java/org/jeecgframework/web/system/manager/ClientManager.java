...
public class ClientManager implements Serializable{
	...
	*/  
	private static final long serialVersionUID = 5018851384799029921L;
	...
	public void addClinet(String sessionId,Client client){
		...
		byte[] clientByte=SerializeUtil.serialize(client);
		ContextHolderUtils.getSession().setAttribute(sessionId, clientByte);
		if(client !=null){
	...
	public Client getClient(String sessionId){
		if(!StringUtils.isEmpty(sessionId)&&ContextHolderUtils.getSession().getAttribute(sessionId)!=null){
			...
			byte[] clientByte =(byte[]) ContextHolderUtils.getSession().getAttribute(sessionId);
			if(clientByte != null && clientByte.length>0) {
				Client client = (Client) SerializeUtil.unserialize(clientByte);
				return client;
			}else {
				return null;
			}			
