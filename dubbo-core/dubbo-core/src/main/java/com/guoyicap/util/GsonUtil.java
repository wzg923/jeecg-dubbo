...
public class GsonUtil {
	...
	* @Title: toMsgModel  
	* @Author:王振广
	* @Description:解析JSON数据，封装为MsgModel 
	* @param jsonStr json数据
	* @param dataClass data类型[List<dataClass>]
	* @return    设定文件  
	* @return Object    返回类型  
	* @throws  
	*/
	public static MsgModel toMsgModel(String jsonStr,Class dataClass){
		Gson gson=GsonUtil.create(null);
		MsgModel model=gson.fromJson(jsonStr,MsgModel.class);
		//处理data
		if(model.getData() !=null && dataClass !=null){
			//解析Object
			if(model.getData() instanceof LinkedTreeMap){
				Map dataMap=(Map) model.getData();
				model.setData(gson.fromJson(gson.toJson(dataMap), dataClass));
			}else{
				//解析List
				List dataList=(List) model.getData();
				//解析数组json
				List list=GsonUtil.jsonToArrayList(gson.toJson(dataList), dataClass,null);
				model.setData(list);
			}
		}		
