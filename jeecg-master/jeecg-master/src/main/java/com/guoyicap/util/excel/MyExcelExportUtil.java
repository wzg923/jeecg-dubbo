...
import com.guoyicap.util.BeanUtils;
...
public class MyExcelExportUtil <T> {
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, String title, List<T> dataList,
			DataGrid dataGrid,String  dateFormatPattern) {
		SimpleDateFormat dateFormat=DateUtils.getSDFormat(dateFormatPattern);
		//如果导出数据超过10W条，则返回错误信息，页面跳转
		if(dataList.size()>100000){
			LogUtil.warning("导出数据超过10万条！");
			request.setAttribute("error", "导出数据超过10万条！请缩小查询范围，分批次导出。");
			//转到404页面 继承BaseController ,父类方法
			...
			try {
				...
				if ("MSIE".equalsIgnoreCase(browse.substring(0, 4))) {
					...
					// 文件名转码
					String newtitle = BrowserUtils.processFileName(request, excelData.getTitle());
					// 设置response 头信息
					response.setHeader("content-disposition", "attachment;filename=" + newtitle + ".xlsx");
				}
				// 产生工作簿对象
				Workbook workbook = null;
				LogUtil.info("开始导出："+new java.util.Date().getTime());
				// 导出Excel
				workbook = ExcelTemplate.export(excelData, dataList.size(), 1048500,dateFormat);
				LogUtil.info("导出Workbook完成："+new java.util.Date().getTime());
				// 获得输出流
	...
	private void getExcelDataMap(List<T> dataList, List<Map<String, Object>> dataMapList) throws MyException {
		...
		if (dataList != null && dataList.size() > 0) {
			for (int i = 0; i < dataList.size(); i++) {
				if(dataList.get(i) instanceof HashMap){
					dataMapList.add((Map<String, Object>) dataList.get(i));
				}else{
