...
import com.sun.star.beans.IntrospectionException;
...
//	public static List<String> encryptFields = Arrays.asList((ResourceUtil.getConfigByName("db.security.fields") == null
//			? "" : (String) ResourceUtil.getConfigByName("db.security.fields")).split(","));
	...
	public static void toJavaBean(Object dest, Object orig) throws MyException  {
					...
					if (PropertyUtils.isWriteable(dest, name)) {
						...
						if (value != null) {
							...
							try {
								...
								e.printStackTrace();
								throw new MyException("MapToBean方法IllegalAccessException",e);
							} catch (InvocationTargetException e) {
								e.printStackTrace();
								throw new MyException("MapToBean方法InvocationTargetException",e);
							} catch (NoSuchMethodException e) {
								e.printStackTrace();
								throw new MyException("MapToBean方法NoSuchMethodException",e);
							}
							if (null == clazz) {
				                  continue;
				              }
				            ...
				            //add by guang  增加特殊变量 数据解密处理  start
				            //TODO
				            /*if(encryptFields.contains(StringUtils.upperCase(name))){
				            	if(StringUtil.isNotEmpty(value)){
				            		try {
										value=SecurityHelper.decryptByPBEWithMD5AndDES("", String.valueOf(value));
									} catch (Exception e) {
										LogUtil.error("数据解密失败。"+name+":"+value,e);
									}
				            	}				            	
				            }*/				              
				            //add by guang  增加特殊变量 数据解密处理  end
