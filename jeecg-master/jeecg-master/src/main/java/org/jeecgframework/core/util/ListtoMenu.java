...
//		menuString.append(icon);
	...
	private static String getHplusSubMenu(TSFunction parent, int level, Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);//取该等级的菜单list
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){//没有子菜单
					menuString.append(getLeafOfHplusTree(function,level,map));
				}else{
					menuString.append(getLeafOfHplusTree(function,level,map));
	...
	private static String getLeafOfHplusTree(TSFunction function, int level,Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getTSIcon() != null) {
			icon = ResourceUtil.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
...
//		menuString.append("<li> <a class=\"J_menuItem\" href=\"").append(function.getFunctionUrl()).append("\">");
		if(StringUtil.isNotEmpty(function.getFunctionUrl())){
			...
			menuString.append("<li> <a class=\"J_menuItem\" ").append(">");
		}
