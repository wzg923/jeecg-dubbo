...
public class CommonDao extends GenericBaseCommonDao implements ICommonDao, IGenericBaseCommonDao {
	...
	public TSUser getUserByUserIdAndUserNameExits(TSUser user) {
		...
		String password=PasswordUtil.encryptWithMD5(user.getPassword());
		//查询status=1 激活用户，否则不可登录 -1：超级管理员  0：禁用 2：离职
		String query = "from TSUser u where u.userName = :username and u.password=:passowrd and u.status in(1,-1) ";
